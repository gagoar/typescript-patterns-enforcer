...
| **Mechanical hook** | A bundled, self-contained checker (`engine/`) runs automatically after every `.ts`/`.tsx` edit via a `PostToolUse` hook, backstopping 7 of the rules below deterministically. |

## Mechanical checks

Seven rules — no `any`, `@ts-ignore` without a description, magic numbers,
`.then()` chains, `switch` statements, parameter mutation, and raw loops —
are also enforced by a small bundled linting engine, not just LLM prose. It
is **fully self-contained**: it ships its own pinned ESLint + parser +
`typescript`, and never reads or writes whatever ESLint/tsconfig setup (if
